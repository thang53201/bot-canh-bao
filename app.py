...
# Cache lưu trữ toàn bộ (Bao gồm cả Vàng để chống lỗi về 0)
GLOBAL_CACHE = {
...
# ==============================================================================
# 2. HÀM LẤY VÀNG (ĐA NGUỒN + CHỐNG VỀ 0)
# ==============================================================================
def get_gold_binance():
    try:
        ...
        d = r.json()
        k = requests.get("https://api.binance.com/api/v3/klines?symbol=PAXGUSDT&interval=1h&limit=20", timeout=15)
        kd = k.json()
        ...
        return {
            ...
            'pct': float(d['priceChangePercent']), 'h1': h1, 'rsi': curr_rsi, 'src': 'Binance'
        }
    except: return None

def get_gold_coingecko():
    """Nguồn dự phòng nếu Binance chết"""
    try:
        url = "https://api.coingecko.com/api/v3/simple/price?ids=pax-gold&vs_currencies=usd&include_24hr_change=true"
        r = requests.get(url, timeout=10)
        d = r.json()['pax-gold']
        # CoinGecko ko có nến H1, nên RSI/H1 giữ nguyên cache cũ hoặc mặc định
        return {
            'p': d['usd'], 'c': d['usd'] * (d['usd_24h_change']/100), 
            'pct': d['usd_24h_change'], 'h1': 0, 'rsi': 50, 'src': 'CoinGecko (Backup)'
        }
    except: return None
...
def update_gold_data():
    ...
    # 2. Nếu Binance lỗi, thử CoinGecko
    if not new_gold:
        new_gold = get_gold_coingecko()
        
    # 3. Cập nhật vào Cache nếu có dữ liệu mới
    if new_gold:
        # Nếu dùng nguồn backup (CoinGecko) ko có RSI, thì giữ RSI cũ
        if new_gold['rsi'] == 50 and GLOBAL_CACHE['gold']['rsi'] != 50:
            new_gold['rsi'] = GLOBAL_CACHE['gold']['rsi']
            new_gold['h1'] = GLOBAL_CACHE['gold']['h1']
            
        GLOBAL_CACHE['gold'] = new_gold
    else:
        # Nếu CẢ 2 ĐỀU LỖI: Giữ nguyên giá cũ, chỉ đổi tên nguồn để cảnh báo
        if GLOBAL_CACHE['gold']['p'] > 0:
            GLOBAL_CACHE['gold']['src'] = "Mất kết nối (Dữ liệu cũ)"
        # Nếu chưa có dữ liệu bao giờ (lần chạy đầu tiên) thì đành chịu 0

# ==============================================================================
# 3. CÁC HÀM VĨ MÔ (GIỮ NGUYÊN)
# ==============================================================================
...
# ==============================================================================
@app.route('/')
def home(): return "Bot V53 - Never Zero Gold"

@app.route('/test')
def run_test():
    update_gold_data() # Force update
...
def run_check():
    try:
        ...
        if is_time and (now - last_sent > 1200):
            ...
            gold_price_txt = f"{gold['p']:.1f}" if gold['p'] > 0 else "N/A"
            ...
            msg = (
                ...
                f"-------------------------------\n"
                f"🥇 <b>GOLD (PAXG):</b> {gold_price_txt}\n"
