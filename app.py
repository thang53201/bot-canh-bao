...
def get_gold_forex_api():
    """Dùng API Twelve Data để lấy giá Spot chuẩn Exness"""
    try:
        ...
        if 'close' in d:
            # Lấy thêm thông số kỹ thuật (RSI) từ Binance (để tiết kiệm request API Forex)
...
def get_gold_binance_tech():
    """Lấy RSI và H1 từ Binance (Nhanh & Free)"""
    try:
...
def get_gold_final():
    ...
    # Nếu lỗi thì dùng Binance
    if not gold: gold = get_gold_binance_full()
    
    # Nếu vẫn lỗi thì dùng Cache cũ
    if not gold:
...
# ==============================================================================
# 3. HÀM LẤY VĨ MÔ (SỬA LẠI ĐỂ KHÔNG BỊ CHẶN)
# ==============================================================================
def get_yahoo_data(symbol):
    """Dùng requests trực tiếp thay vì yfinance"""
    try:
...
# ==============================================================================
@app.route('/')
def home(): return "Bot V87 - Light & Stable"
