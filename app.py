...
CONFIG = {
    ...
    # Số giá chênh lệch giữa PAXG và XAUUSD (Thường PAXG cao hơn ~20-30 giá)
    # Bot sẽ tự trừ đi số này để giá hiển thị khớp Exness hơn
    "GOLD_SPREAD_ADJUST": 25.0 
}
...
# ==============================================================================
# 2. VÀNG BINANCE (CÓ ĐIỀU CHỈNH GIÁ CHO KHỚP EXNESS)
# ==============================================================================
def get_gold_stable():
    try:
        # Nguồn Binance (Bất tử - Không bao giờ bị chặn)
        ...
        # Điều chỉnh giá
        raw_price = float(data['lastPrice'])
        adj_price = raw_price - CONFIG['GOLD_SPREAD_ADJUST'] # Trừ đi chênh lệch

        return {
            'p': adj_price, 
            'c': float(data['priceChange']), 
            'pct': float(data['priceChangePercent']),
            'h1': h1, 'rsi': curr_rsi, 'src': 'Binance (Adj)'
        }
    except: return None

# ==============================================================================
# 3. MACRO YAHOO (5 PHÚT/LẦN)
# ==============================================================================
def get_yahoo_data(symbol):
    try:
        # Random User Agent cực mạnh
        uas = [
...
def update_macro_data():
    ...
    if res10:
        ...
        GLOBAL_CACHE['be_source'] = "Lạm phát (Chờ...)" # Giữ nguyên cache cũ nếu có
...
# ==============================================================================
@app.route('/')
def home(): return "Bot V50 - Stable Hybrid"
