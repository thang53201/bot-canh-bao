...
CONFIG = {
    ...
    # Lạm phát (Điểm số): 10Y > 0.25, 2Y/5Y > 0.20
    "INF_10Y_LIMIT": 0.25,
    "INF_05Y_LIMIT": 0.20,
    
    # FedWatch (Yield Proxy): Thay đổi > 15%
    "FED_PCT_LIMIT": 15.0,
    
    "ALERT_COOLDOWN": 3600
}

# Cache Vĩ mô (5 phút cập nhật 1 lần)
GLOBAL_CACHE = {
    'vix': {'p': 0, 'c': 0, 'pct': 0},
    'gvz': {'p': 0, 'c': 0, 'pct': 0},
    'inf10': {'p': 0, 'c': 0}, # Lạm phát 10Y
    'inf05': {'p': 0, 'c': 0}, # Lạm phát 5Y (Proxy 2Y)
...
def run_check():
    try:
        ...
        if vn_now.minute in [0, 1, 30, 31]:
            ...
            # Hàm format an toàn (tránh lỗi nếu data = 0)
            def fmt(val, chg, pct):
                if val == 0: return "N/A"
                return f"{val:.2f} ({s(pct)}{pct:.2f}%)" # Hiển thị % cho VIX/GVZ/Fed
            
            def fmt_pts(val, chg):
                if val == 0: return "N/A"
                return f"{val:.2f}% (Chg: {s(chg)}{chg:.3f})" # Hiển thị điểm cho Lạm phát
